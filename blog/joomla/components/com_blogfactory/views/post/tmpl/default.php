...
<div class="post-info muted" class="muted">
  ...
  <a href="<?php echo BlogFactoryRoute::task('blog.subscribe&id=' . $this->item->blog_id); ?>" rel="<?php echo $this->subscribed; ?>" class="button-subscribe">
  ...
  <?php if ($this->ratingsEnabled): ?>
    <div class="post-rating">
      ...
      <?php elseif ($this->vote): ?>
        <i class="factory-icon-thumb-up"></i><span class="<?php echo 1 == $this->vote->vote ? 'voted' : ''; ?>"><?php echo $this->item->votes_up; ?></span>
        <i class="factory-icon-thumb-down"></i><span class="<?php echo -1 == $this->vote->vote ? 'voted' : ''; ?>"><?php echo $this->item->votes_down; ?></span>.
      <?php else: ?>
        <a href="<?php echo BlogFactoryRoute::task('post.vote&format=raw&vote=up&id=' . $this->item->id); ?>" class="vote-up"><i class="factory-icon-thumb-up"></i><span><?php echo $this->item->votes_up; ?></span></a>
        <a href="<?php echo BlogFactoryRoute::task('post.vote&format=raw&vote=down&id=' . $this->item->id); ?>" class="vote-down"><i class="factory-icon-thumb-down"></i><span><?php echo $this->item->votes_down; ?></span></a>.
      <?php endif; ?>
    </div>
  <?php endif; ?>
