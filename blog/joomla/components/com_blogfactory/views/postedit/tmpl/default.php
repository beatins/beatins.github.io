...
<form action="<?php echo BlogFactoryRoute::task('post.save&id=' . (int)$this->item->id); ?>" method="POST" name="post-edit" id="post-edit" data-id="<?php echo (int)$this->item->id; ?>" data-revision="<?php echo $this->item->isRevision ? 'true' : 'false'; ?>" novalidate="novalidate">
  ...
  <div style="margin-bottom: 10px; overflow: hidden;">
    <?php if ($this->mediaManagerEnabled): ?>
      <a href="#" class="btn button-add-media"><i class="factory-icon-image-plus"></i><?php echo BlogFactoryText::_('postedit_button_add_media'); ?></a>
    <?php endif; ?>

    <?php if ($this->externalSources): ?>
      <a href="<?php echo BlogFactoryRoute::view('integrations&tmpl=component'); ?>" class="modal btn button-add-integration" rel="{ handler: 'iframe', size: { x: 800, y: 600 } }"><i class="factory-icon-document-horizontal-text"></i><?php echo BlogFactoryText::_('postedit_button_add_integration'); ?></a>
    <?php endif; ?>

    <div style="float: right;" class="blogfactory-post-submit">
      <?php if ($this->item->id): ?>
        <a href="#" class="btn button-preview"><?php echo BlogFactoryText::_('postedit_button_preview_changes'); ?></a>
      <?php endif; ?>

      <?php if (!$this->item->published): ?>
        <a href="#" class="btn button-draft" rel="draft"><?php echo BlogFactoryText::_('postedit_button_save_draft'); ?></a>
      <?php endif; ?>

      <a href="#" class="btn btn-primary button-save" rel="<?php echo $this->item->published ? 'update' : 'publish'; ?>">
        <?php if ($this->item->published): ?>
  ...
  <?php if ($this->item->id): ?>
    <div style="margin-top: 10px;" class="muted">
      ...
      <div class="autosave" style="float: right;">
        ...
        <span class="loader" style="display: none;"><?php echo BlogFactoryText::_('postedit_autosaving'); ?></span>
      </div>
    </div>
  <?php endif; ?>
