...
class BlogFactoryHtml
{
  ...
  {
    if (!self::getCompress() || in_array($file, array('tinymce/tinymce', 'tinymce/tinymce.jquery'))) {
...
//    }
  ...
  protected static function getCompress()
  {
    ...
    if (is_null(self::$compress)) {
      ...
      $result = $extension->find(array('type' => 'plugin', 'element' => 'blogfactory', 'folder' => 'system', 'enabled' => 1));

      self::$compress = (boolean)$result;
    }
