...
class JHtmlBlogFactoryComments
{
  ...
  protected static function displayAddReply($postId, $config)
  {
    ...
    if ($authorised) {
      ...
      if ($settings->get('comments.enable.guest_comments', 1) || !$user->guest) {
        ...
        $html[] = BlogFactoryText::_('comments_leave_reply_error_guests_not_allowed');
      }
  ...
  protected static function getListQuery($postId, $total = false)
  {
    ...
    if ($settings->get('comments.approval', 0)) {
      $query->where('(c.approved = ' . $dbo->quote(1) . ' OR c.user_id = ' . $dbo->quote($user->id) . ')');
    }
