...
class BlogFactoryFrontendModelPostEdit extends JModelLegacy
{
  ...
  public function save($data, $mode = 'draft')
  {
    ...
    if ($table->published && $table->visibility &&
    ...
    if ($table->published) {
      ...
      $dispatcher->trigger('onBlogFactoryPostPublished', array('com_blogfactory.post.published', $table));
    }
  ...
  protected function exportContent($table, $blog)
  {
    // Initialise variables.
    $settings       = JComponentHelper::getParams('com_blogfactory');
    $settingsExport = (array)$settings->get('export');
    $blogExport     = new JRegistry($blog->export);
