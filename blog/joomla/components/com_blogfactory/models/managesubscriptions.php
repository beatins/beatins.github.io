...
class BlogFactoryFrontendModelManageSubscriptions extends JModelLegacy
{
  protected $start;
  protected $limit = 10;
  ...
  protected function getQuery($total = false)
  {
    ...
    $query = $dbo->getQuery(true)
      ...
      ->leftJoin('#__com_blogfactory_blogs b ON b.id = s.blog_id')
      ->where('s.user_id = ' . $dbo->quote($user->id));

    if ($total) {
      $query->select('COUNT(s.id) AS total');
    } else {
      $query->select('b.id, b.title, b.alias, s.created_at');
  ...
  protected function addQueryOrderItems($query)
  {
    ...
    switch ($sort) {
      ...
      case 'date':
        $query->order('s.created_at ' . $order);
        break;
