...
foreach ($this->items as $this->i => $this->item): ?>
  <tr>
    ...
    <td class="nowrap has-context">
      <a href="<?php echo BlogFactoryRoute::task('user.edit&id=' . $this->item->id); ?>" title="<?php echo JText::_('JACTION_EDIT'); ?>">
        <?php echo $this->escape($this->item->username); ?></a>
    </td>

    <td class="nowrap has-context">
      <?php if ($this->item->blog_id): ?>
        <a href="<?php echo BlogFactoryRoute::task('blog.edit&id=' . $this->item->blog_id); ?>" title="<?php echo JText::_('JACTION_EDIT'); ?>">
