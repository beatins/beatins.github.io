...
class BlogFactoryHelper
{
  ...
  public static function filterPostContent($text)
  {
    ...
    $attrArray  = array('src', 'style', 'href', 'height', 'width', 'target', 'class', 'alt');
    $tagsMethod = 0;
    $attrMethod = 0;
    $xssAuto    = 0;

    $filter = JFilterInput::getInstance($tagsArray, $attrArray, $tagsMethod, $attrMethod, $xssAuto);
  ...
  protected static function checkPermission($permission)
  {
    ...
    switch ($permission) {
      ...
      case 'create_blog':
        if (self::isLoveFactoryIntegrated()) {
          if (!self::isUserAllowedByLoveFactory()) {
            $redirect = BlogFactoryRoute::view('blogs');
            $message  = BlogFactoryText::_('permission_lovefactory_create_blog_message');
          }
        }
        elseif (self::isSocialFactoryIntegrated()) {
  ...
  protected static function isLoveFactoryIntegrated()
  {
  ...
  protected static function isUserAllowedByLoveFactory()
  {
    ...
    if (!$user->id || !$profile->load($user->id)) {
      return false;
    }

    $membership->load($profile->membership_sold_id);

    return $membership->blogfactory;
  }
